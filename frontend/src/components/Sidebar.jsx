...
const Sidebar = ({ user, open, onClose, onLogout }) => {
  const links = [
    user?.role === "staff" ? { label: "Staff Panel", to: "/staff" } : null,
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed left-0 top-0 z-40 flex h-screen w-[290px] flex-col border-r border-white/10 bg-slate-900 text-white transition-transform duration-300 md:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}
      >
